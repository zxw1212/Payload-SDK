...
void* ForwardStatusMqtt_Task(void* arg) {
  ...
  try {
    auto top_base = mqtt::topic(*cli, s_CLIENT_ID + "/status/base", kNum_QOS);
    auto top_aircraft =
        mqtt::topic(*cli, s_CLIENT_ID + "/status/aircraft", kNum_QOS);
    auto top_rtk = mqtt::topic(*cli, s_CLIENT_ID + "/status/rtk", kNum_QOS);
    auto top_battery =
        mqtt::topic(*cli, s_CLIENT_ID + "/status/aircraft_battery", kNum_QOS);
    auto top_gimbal = mqtt::topic(*cli, s_CLIENT_ID + "/status/gimbal", kNum_QOS);
    auto top_waypoint =
    ...
    while (true) {
      ...
      ofstream ofs;
      ofs.open("log.txt", ios::app);  // 采取追加的方式写入文件
      ...
      ofs << std::to_string(velocity.data.z) << endl;
      ofs.close();

      // TODO jv_aircraft["aircraftVideoSignal"] = std::to_string(0);
      jv_aircraft["flightMode"] = getFlightModestd::string(displayMode);
      ...
      if (homePointInfo.longitude > -3.2 && homePointInfo.longitude < 3.2) {
        jv_aircraft["homeLocationLatitude"] =
            std::to_string(homeInfo.latitude / DEG2RAD);
        jv_aircraft["homeLocationLongitude"] =
            std::to_string(homeInfo.longitude / DEG2RAD);
      ...
      /*
         ...
         << "\n"
                      << rtk_velocity.x << ","<< rtk_velocity.y<< "," <<
         rtk_velocity.z
                      << "," << rtk_yaw << "," << (uint16_t)rtk_yaw_info <<","<<
         (uint16_t)rtk_pos_info<< "\n";
      */
      // 5.3.3 [/status/rtk]
#if 1
      if (1)  // TODO{zengxw} can not get rtkAvailable flag
      {
        Json::Value jv_rtk;
      ...
      if (countSecond++ % 2 == 0) {
        ...
        // TODO{zengxw} check this flag, FYI@Guojj
